...
MCP_MODULES_DIR = Path(__file__).resolve().parent.parent / "mcp_modules"
...
def validate_module_structure(module_path: Path) -> list[str]:
    """
    Validates the module boilerplate and returns a list of detailed errors.
    Optimized for LLM feedback.
    """
    errors = []
    ...
    if not errors:
        # 3. Check capability implementation
        try:
            # We use a unique name for the spec to avoid cache issues during validation
...
def delete_module_files(slug: str, db: Session):
    mod = db.query(MCPModule).filter(MCPModule.slug == slug).first()
...
def discover_modules():
    modules = []
    if not MCP_MODULES_DIR.exists():
        return modules
    for item in MCP_MODULES_DIR.iterdir():
...
def sync_modules_to_db(db: Session):
    ...
    db.query(MCPModule).filter(MCPModule.slug.notin_(discovered_slugs)).delete(synchronize_session=False)

    for m in discovered:
        slug = m.get("slug", "")
        existing = db.query(MCPModule).filter(MCPModule.slug == slug).first()
        if not existing:
            mod = MCPModule(
...
def load_module(slug: str, db: Session):
    mod_record = db.query(MCPModule).filter(MCPModule.slug == slug, MCPModule.is_active == True).first()
    ...
    # Hot-Reload Unterstützung für MCP Module
    if slug in _loaded_modules:
...
def execute_read(slug: str, db: Session, method: str, params: dict = None):
    ...
    # Pass db session to allow modules to interact with OpenAMS data structures
    res = fn(p, db)
    if isinstance(res, dict) and res.get("status") == "error":
        res["_debug"] = {
            "db_config_injected": p["config"],
            "params_received": params,
            "hint": "Check if API key is empty in the UI (MCP-Module -> Konfigurieren)"
        }
    return res
